...
async function addFriend(username){
  ...
  const id=`${user.uid}_${to}`;
  // An uncreated request has no resource.data; reading it by ID is denied by
  // the private friendRequests get rule. Check our own visible requests instead.
  const sent=await getDocs(query(collection(db,'friendRequests'),where('from','==',user.uid)));
  if(sent.docs.some(item=>item.id===id))throw new Error('برای این کاربر قبلاً درخواست فرستادی.');
  // Firestore rules permit creating this request, not overwriting an existing one.
  await setDoc(doc(db,'friendRequests',id),{from:user.uid,to,status:'pending'});
