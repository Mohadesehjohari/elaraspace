...
async function init(page,{seed=false}={}){
 ...
 const bootPaint=await page.evaluate(()=>window.__elaraBootPaint||[]);assert.equal(bootPaint.some(x=>x.booting&&x.visibility!=='hidden'),false,'legacy shell painted before final shell readiness');
...
for(const [width,height] of sizes){
 ...
 try{
  ...
  if(width===390){
    ...
    let state=await page.evaluate(()=>JSON.parse(localStorage.getItem('elara_space_v1')));assert.equal(state.tasks[0].completed,true);assert.ok(state.xp>=10,'canonical task XP missing');assert.equal(state.tasks[0].xpAwarded,true);
    assert.equal(await page.locator('.ref-task-check[data-ref-task="qa-task"]').getAttribute('aria-pressed'),'true');
    await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.ElaraReferenceHome&&!document.documentElement.hasAttribute('data-elara-booting'));await page.evaluate(()=>{document.body.classList.add('cloud-ready');document.body.classList.remove('cloud-locked');document.getElementById('cloud-layer')?.setAttribute('hidden','');window.ElaraOpen('home',{history:'replace'});window.ElaraReferenceHome.render()});await page.waitForFunction(()=>document.querySelector('.ref-task-check[data-ref-task="qa-task"]'));await page.waitForTimeout(120);
    ...
    const xpBeforeUntoggle=await page.evaluate(()=>JSON.parse(localStorage.getItem('elara_space_v1')).xp);await page.locator('.ref-task-check[data-ref-task="qa-task"]').click();state=await page.evaluate(()=>JSON.parse(localStorage.getItem('elara_space_v1')));assert.equal(state.tasks[0].completed,false);assert.equal(state.xp,xpBeforeUntoggle,'unchecking must not award XP');
    await page.waitForFunction(()=>document.querySelector('[data-home-goal-step="qa-step"]'));const step=page.locator('[data-home-goal-step="qa-step"]');assert.equal(await step.count(),1,'real goal step control absent');await step.click();state=await page.evaluate(()=>JSON.parse(localStorage.getItem('elara_space_v1')));assert.equal(state.goals[0].steps[0].done,true);
    assert.equal(await page.locator('#ref-header-account .ref-account-equipped-frame').count(),1,'equipped topbar frame missing');await page.evaluate(()=>{localStorage.setItem('elara_visual_wardrobe_guest',JSON.stringify({frame:null}));window.dispatchEvent(new CustomEvent('elara:wardrobe-changed',{detail:{uid:'guest',wardrobe:{frame:null}}}))});await page.waitForTimeout(220);assert.equal(await page.locator('#ref-header-account .ref-account-equipped-frame').count(),0,'unequipped frame remained stuck in Topbar');
