...
assert.match(nav,/const SIDEBAR=Object\.freeze\(\[\.\.\.MAIN,FRIEND\]\);/,'Desktop Sidebar must retain the real Friends route');
assert.match(nav,/\['home','tasks','language','books','exercise','ranking','social','freedom'\]/,'Friends must follow ranking before freedom');
assert.match(nav,/route:'social',label:'دوستان'/,'Friends must remain a real primary route');
...
console.log('PASS: desktop Sidebar retains Friends, seven-route MAIN plus Friends extension remain compatible, Reports secondary and Drawer preservation.');
