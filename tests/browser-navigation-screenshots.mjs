...
const dataPage=await browser.newPage({viewport:{width:390,height:844},deviceScaleFactor:1});await ready(dataPage,{seed:true});const dataRows=await dataPage.locator('.ref-task-row').count();assert.ok(dataRows>0&&dataRows<=3,'Mobile data preview must be capped without deleting data');measurements.push({mode:'seeded-data',viewport:390,header:await bounds(dataPage,'.topbar'),hero:await bounds(dataPage,'.ref-hero'),streak:await bounds(dataPage,'#ref-streak-card'),tasks:await bounds(dataPage,'.ref-tasks'),habits:await bounds(dataPage,'.ref-habits'),wellness:await bounds(dataPage,'.ref-wellness-card'),missions:await bounds(dataPage,'.ref-missions'),ranks:await bounds(dataPage,'.ref-ranks'),friends:await bounds(dataPage,'.ref-activity'),bottomNav:await bounds(dataPage,'.bottom-nav'),metrics:await pageMetrics(dataPage)});await dataPage.screenshot({path:out+'/after-mobile-390-data-viewport.png',fullPage:false});await dataPage.screenshot({path:out+'/after-mobile-390-data-full.png',fullPage:true});await dataPage.close();
