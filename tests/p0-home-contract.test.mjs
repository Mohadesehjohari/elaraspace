...
assert.match(nav,/const SIDEBAR=Object\.freeze\(\[\.\.\.MAIN\]\);/,'Desktop Sidebar must exclude Friends from primary navigation');
assert.match(nav,/const DESKTOP_ORDER=Object\.freeze\(\['home','tasks','language','books','exercise','ranking','freedom'\]\);/,'Desktop order must contain only primary non-Friends routes');
assert.doesNotMatch(nav,/route:'social'/,'Friends must not be a primary navigation definition');
for(const forbidden of ["route:'habits'","route:'goals'","route:'focus'"])assert.doesNotMatch(nav.split('const SIDEBAR=Object.freeze([...MAIN]);')[1]||'',new RegExp(forbidden));
...
console.log('PASS: canonical navigation without primary Friends, artwork-backed reference Home with Friends-card entry, compact mobile preview, Library Focus, Wellness boundary and Drawer layering guards.');
