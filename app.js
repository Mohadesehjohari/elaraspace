(() => {
  ...
  const initial = () => ({version:1, tasks:[], habits:[], goals:[], books:[], words:[], folders:[], tags:[], xp:0, theme:'dark'});
  ...
  function handleTaskClick(event) {
    ...
    if(button.dataset.action==='delete-task'&&confirm('این تسک حذف شود؟')){state.tasks=state.tasks.filter(t=>t.id!==task.id);if(editingTask===task.id)resetTaskForm();save();renderTasks();}
  }
  ...
  function handleHabitClick(event){const b=event.target.closest('[data-action]');if(!b)return;const h=state.habits.find(x=>x.id===b.dataset.id);if(!h)return;if(b.dataset.action==='toggle-habit'){if(h.days.includes(today()))h.days=h.days.filter(x=>x!==today());else{h.days.push(today());if(!h.rewardDays.includes(today())){h.rewardDays.push(today());state.xp+=15;}}save();renderHabits();renderHeader();}if(b.dataset.action==='delete-habit'&&confirm('این عادت حذف شود؟')){state.habits=state.habits.filter(x=>x.id!==h.id);save();renderHabits();}}
  ...
  function handleGoalClick(event){const b=event.target.closest('[data-action]');if(!b)return;const goal=state.goals.find(g=>g.id===b.dataset.goal);if(!goal)return;const action=b.dataset.action;if(action==='delete-goal'&&confirm('هدف و قدم‌های آن حذف شود؟'))state.goals=state.goals.filter(g=>g.id!==goal.id);if(action==='add-step'){const text=asText(prompt('عنوان قدم جدید:'));if(text)goal.steps.push({id:makeId(),text,done:false});}if(action==='toggle-step'){const s=goal.steps.find(x=>x.id===b.dataset.id);if(s)s.done=!s.done;}if(action==='delete-step'&&confirm('این قدم حذف شود؟'))goal.steps=goal.steps.filter(s=>s.id!==b.dataset.id);save();renderGoals();}
  ...
  $('book-list').addEventListener('click',event=>{const b=event.target.closest('[data-action="delete-book"]');if(!b||!confirm('کتاب حذف شود؟'))return;state.books=state.books.filter(book=>book.id!==b.dataset.id);save();renderBooks();});
  ...
  $('word-list').addEventListener('click',event=>{const b=event.target.closest('[data-action="delete-word"]');if(!b||!confirm('کلمه حذف شود؟'))return;state.words=state.words.filter(w=>w.id!==b.dataset.id);save();renderWords();});
