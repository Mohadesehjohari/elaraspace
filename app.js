(() => {
  ...
  function renderHabits(){ $('habit-list').innerHTML=state.habits.map(h=>`<li class="item ${h.days.includes(today())?'done':''}"><button type="button" class="check-button" data-action="toggle-habit" data-id="${esc(h.id)}" aria-pressed="${h.days.includes(today())}" aria-label="ثبت امروز برای ${esc(h.title)}">${h.days.includes(today())?'<img class="elara-check-art" src="assets/ui/icon-tasks-check-alpha.webp" alt="" decoding="async">':''}</button><div class="item-content"><div class="item-title">${esc(h.title)}</div><div class="item-meta"><span>تداوم: ${fmt(streak(h))} روز</span><span>کل روزها: ${fmt(h.days.length)}</span></div></div><button type="button" class="mini-button danger" data-action="delete-habit" data-id="${esc(h.id)}">حذف</button></li>`).join('');$('habit-empty').classList.toggle('hidden',!!state.habits.length);}
