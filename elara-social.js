...
function profileSummary(person){return `<div class="elara-public-head">${avatar(person.name)}<div><h2>${esc(person.name||person.username||'کاربر')}</h2><p>@${esc(person.username||'')} · ${esc(title(person.xp))} · Level ${lv(person.xp)}</p></div><strong>${Number(person.xp||0).toLocaleString('fa-IR')} XP</strong></div>`}
function renderProfile(person=state.profileView){
 ...
 if(self){
  root.innerHTML=`<div class="elara-profile-grid"><section class="elara-card wide">${profileSummary(person)}<p class="muted">این صفحه فقط اطلاعات نمایشی حساب را ویرایش می‌کند؛ Task، Habit، Goal و داده‌های خصوصی وارد پروفایل عمومی نمی‌شوند.</p></section><section class="elara-card wide"><h2>ویرایش پروفایل من</h2><form id="elara-profile-form" class="elara-profile-form"><label>نام نمایشی<input id="elara-profile-edit-name" maxlength="60" required value="${esc(person.name||'')}"></label><label>نام کاربری<input id="elara-profile-edit-username" maxlength="20" required pattern="[a-z][a-z0-9_]{2,19}" value="${esc(person.username||'')}"></label><label class="wide">Bio<textarea id="elara-profile-edit-bio" maxlength="300" rows="4" placeholder="دربارهٔ خودت…">${esc(person.bio||'')}</textarea></label><label class="elara-privacy wide"><input id="elara-profile-public" type="checkbox" ${person.profilePublic!==false?'checked':''}> پروفایل نمایشی من برای کاربران واردشده قابل مشاهده باشد</label><button class="primary-button" type="submit">ذخیره پروفایل</button><span id="elara-profile-save-status" class="muted"></span></form></section><section class="elara-card"><h2>افتخارات</h2><p>${esc(title(person.xp))} · Level ${lv(person.xp)}</p><p class="muted">Medal/Avatar/Frame unlock در ادامهٔ همین فاز تکمیل می‌شود.</p></section><section class="elara-card"><h2>حریم خصوصی</h2><p class="muted">خاموش‌کردن پروفایل عمومی، مشاهدهٔ کامل توسط غیردوستان را محدود می‌کند. دوستان و درخواست‌های در جریان برای نمایش هویت پایه همچنان قابل تشخیص‌اند.</p></section></div>`;
  return;
 }
 if(!publicAllowed){root.innerHTML=`<section class="elara-card">${profileSummary(person)}<p class="muted">این کاربر نمایش عمومی پروفایلش را محدود کرده است.</p></section>`;return}
 let actions='';
 ...
 root.innerHTML=`<div class="elara-profile-grid"><section class="elara-card wide">${profileSummary(person)}<p class="elara-profile-bio">${esc(person.bio||'هنوز Bio ثبت نشده است.')}</p><div class="timer-actions">${actions}</div></section><section class="elara-card"><h2>وضعیت اجتماعی</h2><p>${isFriend?'دوست تأییدشده':relation?.status==='pending'?'درخواست دوستی در جریان است':'هنوز دوست نیستید'}</p><p class="muted">Challenge فقط بعد از دوستی پذیرفته‌شده فعال خواهد شد.</p></section><section class="elara-card"><h2>Level و Title</h2><p>Level ${lv(person.xp)} · ${esc(title(person.xp))}</p><p class="muted">${Number(person.xp||0).toLocaleString('fa-IR')} XP</p></section></div>`;
}
...
async function saveProfileValues(values={}){
 ...
 const profileRef=doc(db,'profiles',uid),before=state.me||{},warnings=[];
 // Name/Bio work with both the old and the new Firestore rules.
 await updateDoc(profileRef,{name,bio});
 await updateProfile(auth.currentUser,{displayName:name});
 if(username!==before.username){
   try{
     await runTransaction(db,async tx=>{
       const own=await tx.get(profileRef);if(!own.exists())throw Error('پروفایل پیدا نشد.');
       const oldUsername=own.data().username,newClaim=doc(db,'usernames',username),claim=await tx.get(newClaim);
       if(claim.exists()&&claim.data().uid!==uid)throw Error('این نام کاربری قبلاً انتخاب شده است.');
       tx.set(newClaim,{uid});tx.update(profileRef,{username});tx.delete(doc(db,'usernames',oldUsername));
     });
   }catch(error){
     if(error.code==='permission-denied')warnings.push('تغییر نام کاربری بعد از انتشار Firestore Rules جدید فعال می‌شود.');
     else throw error;
   }
 }
 if(profilePublic!==(before.profilePublic!==false)){
   try{await updateDoc(profileRef,{profilePublic})}
...
function render(){if(!$('elara-social-page'))return;const incoming=state.requests.filter(r=>r.to===uid&&r.status==='pending'),outgoing=state.requests.filter(r=>r.from===uid&&r.status==='pending');
 ...
 const lines=ranking.map((p,i)=>`<div class="elara-rank-line"><b>${i+1}</b>${avatar(p.name)}${profile(p)}<span>${Number(p.xp||0).toLocaleString('fa-IR')} XP</span></div>`).join('');
 $('elara-ranking-page').innerHTML=`<div class="elara-social-grid"><section class="elara-card wide"><h2>رنکینگ میان من و دوستان پذیرفته‌شده</h2><p class="muted">روی نام هر نفر بزن تا پروفایلش باز شود. XP سمت مرورگر هنوز ضدتقلب رسمی نیست.</p>${lines||'<p class="muted">هنوز حسابی بارگذاری نشده است.</p>'}<button type="button" class="quiet-button" data-social-refresh>به‌روزرسانی</button></section><section class="elara-card wide"><h2>لقب‌های مسیر (۱۰ سطح)</h2><div class="elara-title-grid">${window.ElaraLevels?.titles.map((t,i)=>`<span class="elara-title-pill">${i+1>lv(self?.xp)?'🔒':'✦'} ${i+1} · ${esc(t)}</span>`).join('')||''}</div></section></div>`;
 ...
 $('elara-home-social').innerHTML=`<p>${state.friends.length} دوست تأییدشده · ${incoming.length} درخواست جدید</p><button type="button" data-elara-tab="social" class="primary-button">مشاهده دوستان و درخواست‌ها</button>`;
