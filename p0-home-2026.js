...
function init(){cleanTopbar();ensureSearch();render();watchTimer();document.addEventListener('click',handleActions);document.addEventListener('submit',quickTask);const scheduleRender=()=>setTimeout(render,0);for(const name of ['elara:data-changed','elara:hydrate','elara:account-ready','elara:social-updated','elara:open'])window.addEventListener(name,scheduleRender);window.addEventListener('storage',scheduleRender);setTimeout(cleanTopbar,250);setTimeout(cleanTopbar,1000)}
