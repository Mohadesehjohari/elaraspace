...
function renderTasks(){const host=$('elara-home-tasks');if(!host)return;const s=read(),compact=isMobile(),limit=compact?2:5,items=arr(s.tasks).filter(t=>!done(t)&&due(t)).sort((a,b)=>(Number(a.priority)||4)-(Number(b.priority)||4)).slice(0,limit),all=arr(s.tasks).filter(t=>due(t)),completed=all.filter(t=>done(t)).length;
