...
function renderTasks(){const host=$('elara-home-tasks');if(!host)return;const s=read(),compact=isMobile(),limit=compact?2:5,items=arr(s.tasks).filter(t=>due(t)).sort((a,b)=>Number(done(a))-Number(done(b))||(Number(a.priority)||4)-(Number(b.priority)||4)).slice(0,limit),all=arr(s.tasks).filter(t=>due(t)),completed=all.filter(t=>done(t)).length;
 const empty=compact?'امروز هنوز کاری نداری.':'هنوز کاری برای امروز ثبت نشده است.',footer=compact?'':'<button type="button" class="ref-card-link" data-elara-tab="tasks">+ افزودن کار / مشاهده همه</button>';
 host.innerHTML=`<div class="ref-card-count">${fa(completed)} / ${fa(all.length)} انجام‌شده</div><div class="ref-task-list">${items.length?items.map(t=>`<div class="ref-task-row ${done(t)?'done':''}"><button type="button" data-ref-task="${esc(t.id)}" aria-label="تکمیل ${esc(t.text||t.title||'کار')}" class="ref-task-check" aria-pressed="${done(t)}">${done(t)?'✓':''}</button><strong title="${esc(t.text||t.title||'')}">${esc(t.text||t.title||'بدون عنوان')}</strong><span class="ref-priority p${Math.min(4,Math.max(1,Number(t.priority)||4))}">P${Math.min(4,Math.max(1,Number(t.priority)||4))}</span></div>`).join(''):`<p class="ref-empty">${empty}</p>`}</div>${footer}`}
function renderHabits(){const host=$('elara-home-habits');if(!host)return;const all=arr(read().habits),compact=isMobile(),s=all.slice(0,compact?2:5),d=today(),n=all.filter(h=>arr(h.days).includes(d)).length;
 const empty=compact?'هنوز عادتی ثبت نکردی.':'هنوز عادت شخصی ثبت نشده است.',footer=compact?'':'<button type="button" class="ref-card-link" data-elara-tab="habits">مشاهده همه عادت‌ها ←</button>';
...
function accountHeader(){const bar=document.querySelector('.topbar'),actions=bar?.querySelector('.topbar-actions'),leading=bar?.querySelector('.topbar-leading');if(!bar||!actions)return;bar.classList.add('ref-topbar');document.querySelectorAll('.elara-toolbar').forEach(x=>x.remove());if(leading&&!leading.querySelector('.ref-mobile-brand')){const brand=element('a','ref-mobile-brand','ref-mobile-brand');brand.href='#home';brand.setAttribute('aria-label','Elara Space');brand.innerHTML='<img src="assets/logo.svg" alt=""><span><b>Elara</b><small>زندگی بهتر، هر روز</small></span>';leading.append(brand)}
 ...
 account.classList.toggle('ref-account-brand-duplicate',name.trim().toLocaleLowerCase()==='elara');const frame=String(v.frameSrc||'');account.innerHTML=`<span class="ref-account-avatar">${avatar?`<img src="${esc(avatar)}" alt="">`:esc(name.trim()[0]||'E')}${frame?`<img class="ref-account-equipped-frame" data-src="${esc(frame)}" src="${esc(frame)}" alt="">`:''}</span><span class="ref-account-copy"><strong>${esc(name)}</strong><small>سطح ${fa(level)} · ${fa(xp)} XP</small><span class="ref-xp-track"><i style="width:${Math.min(100,(xp%1000)/10)}%"></i></span></span>`;
