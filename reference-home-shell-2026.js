...
function renderTasks(){const host=$('elara-home-tasks');if(!host)return;const s=read(),items=arr(s.tasks).filter(t=>!done(t)&&due(t)).sort((a,b)=>(Number(a.priority)||4)-(Number(b.priority)||4)).slice(0,5),all=arr(s.tasks).filter(t=>due(t)),completed=all.filter(t=>done(t)).length;
 host.innerHTML=`<div class="ref-card-count">${fa(completed)} / ${fa(all.length)} انجام‌شده</div><div class="ref-task-list">${items.length?items.map(t=>`<div class="ref-task-row"><button type="button" data-ref-task="${esc(t.id)}" aria-label="تکمیل ${esc(t.text||t.title||'کار')}" class="ref-task-check"></button><strong title="${esc(t.text||t.title||'')}">${esc(t.text||t.title||'بدون عنوان')}</strong><span class="ref-priority p${Math.min(4,Math.max(1,Number(t.priority)||4))}">P${Math.min(4,Math.max(1,Number(t.priority)||4))}</span></div>`).join(''):'<p class="ref-empty">هنوز کاری برای امروز ثبت نشده است.</p>'}</div><button type="button" class="ref-card-link" data-elara-tab="tasks">+ افزودن کار / مشاهده همه</button>`}
function renderHabits(){const host=$('elara-home-habits');if(!host)return;const s=arr(read().habits),d=today(),top=s.slice(0,5),n=s.filter(h=>arr(h.days).includes(d)).length;
 host.innerHTML=`<div class="ref-card-count">${fa(n)} / ${fa(s.length)} انجام‌شده</div><div class="ref-habit-list">${top.length?top.map(h=>{const p=arr(h.days).includes(d)?100:0;return `<button type="button" class="ref-habit-row" data-ref-habit="${esc(h.id)}"><span class="ref-habit-mark">${p?'✓':'○'}</span><span class="ref-habit-copy"><strong>${esc(h.title||h.name||'عادت')}</strong><span class="elara-track"><i style="width:${p}%"></i></span></span><b>${fa(p)}٪</b></button>`}).join(''):'<p class="ref-empty">هنوز عادت شخصی ثبت نشده است.</p>'}</div><button type="button" class="ref-card-link" data-elara-tab="habits">مشاهده همه عادت‌ها ←</button>`}
...
function renderMissions(){const host=$('elara-home-missions');if(!host)return;const ms=arr(window.ElaraMissions?.snapshot?.()).slice(0,3);host.innerHTML=ms.length?ms.map(m=>`<div class="ref-mission-row"><span class="ref-mission-check" aria-hidden="true">${m.completed?'✓':'○'}</span><div><strong>${esc(m.name||'مأموریت')}</strong><small>${esc(m.detail||'')} · ${fa(m.amount)} / ${fa(m.target)}</small></div><b>+${fa(m.rewardXp)} XP</b></div>`).join(''):'<p class="ref-empty">مأموریتی برای نمایش نیست.</p>'}
...
function accountHeader(){const bar=document.querySelector('.topbar'),actions=bar?.querySelector('.topbar-actions'),leading=bar?.querySelector('.topbar-leading');if(!bar||!actions)return;bar.classList.add('ref-topbar');document.querySelectorAll('.elara-toolbar,.topbar-actions .elara-profile').forEach(x=>x.remove());if(leading&&!leading.querySelector('.ref-mobile-brand')){const brand=element('a','ref-mobile-brand','ref-mobile-brand');brand.href='#home';brand.setAttribute('aria-label','Elara Space');brand.innerHTML='<img src="assets/logo.svg" alt=""><span><b>Elara</b><small>زندگی بهتر، هر روز</small></span>';leading.append(brand)}
