...
function homeStructure(){
 ...
 panel.classList.add('ref-home');grid.classList.add('ref-home-grid');
 ...
 let wellness=$('ref-wellness-card');if(!wellness){wellness=element('section','elara-card ref-card ref-wellness-card','ref-wellness-card');wellness.innerHTML=`<header><h2>${icon('workout')} سلامت / ورزش</h2><button type="button" class="elara-link" data-elara-tab="exercise">همه ←</button></header><div id="ref-wellness-data"></div>`}
 ...
 for(const c of [tasks,habits,wellness,missions,goals])if(c){c.hidden=false;grid.append(c)}
 for(const c of [ranks,activity])if(c){c.hidden=false;side.append(c)}
...
function libraryFocus(){
 ...
 let section=$('ref-library-focus');if(!section){section=element('section','elara-card ref-library-focus','ref-library-focus');section.innerHTML=`<header><h2>${icon('focus')} فضای تمرکز و پومودورو</h2><span class="muted">جلسه‌ها و زمان‌سنج فعلی</span></header>`;books.append(section)}
...
function renderTasks(){const host=$('elara-home-tasks');if(!host)return;const s=read(),compact=isMobile(),limit=compact?3:5,items=arr(s.tasks).filter(t=>!done(t)&&due(t)).sort((a,b)=>(Number(a.priority)||4)-(Number(b.priority)||4)).slice(0,limit),all=arr(s.tasks).filter(t=>due(t)),completed=all.filter(t=>done(t)).length;
 const empty=compact?'امروز هنوز کاری نداری.':'هنوز کاری برای امروز ثبت نشده است.',footer=compact?(items.length?'':'<button type="button" class="ref-card-link" data-elara-tab="tasks">+ افزودن</button>'):'<button type="button" class="ref-card-link" data-elara-tab="tasks">+ افزودن کار / مشاهده همه</button>';
 host.innerHTML=`<div class="ref-card-count">${fa(completed)} / ${fa(all.length)} انجام‌شده</div><div class="ref-task-list">${items.length?items.map(t=>`<div class="ref-task-row"><button type="button" data-ref-task="${esc(t.id)}" aria-label="تکمیل ${esc(t.text||t.title||'کار')}" class="ref-task-check"></button><strong title="${esc(t.text||t.title||'')}">${esc(t.text||t.title||'بدون عنوان')}</strong><span class="ref-priority p${Math.min(4,Math.max(1,Number(t.priority)||4))}">P${Math.min(4,Math.max(1,Number(t.priority)||4))}</span></div>`).join(''):`<p class="ref-empty">${empty}</p>`}</div>${footer}`}
function renderHabits(){const host=$('elara-home-habits');if(!host)return;const all=arr(read().habits),compact=isMobile(),s=all.slice(0,compact?3:5),d=today(),n=all.filter(h=>arr(h.days).includes(d)).length;
 const empty=compact?'هنوز عادتی ثبت نکردی.':'هنوز عادت شخصی ثبت نشده است.',footer=compact?(s.length?'':'<button type="button" class="ref-card-link" data-elara-tab="habits">+ افزودن</button>'):'<button type="button" class="ref-card-link" data-elara-tab="habits">مشاهده همه عادت‌ها ←</button>';
 host.innerHTML=`<div class="ref-card-count">${fa(n)} / ${fa(all.length)} انجام‌شده</div><div class="ref-habit-list">${s.length?s.map(h=>{const p=arr(h.days).includes(d)?100:0;return `<button type="button" class="ref-habit-row" data-ref-habit="${esc(h.id)}"><span class="ref-habit-mark">${p?'✓':'○'}</span><span class="ref-habit-copy"><strong>${esc(h.title||h.name||'عادت')}</strong><span class="elara-track"><i style="width:${p}%"></i></span></span><b>${fa(p)}٪</b></button>`}).join(''):`<p class="ref-empty">${empty}</p>`}</div>${footer}`}
function renderGoals(){const host=$('elara-home-goals');if(!host)return;const goals=arr(read().goals).slice(0,4);host.innerHTML=goals.length?goals.map(g=>{const a=arr(g.steps),p=a.length?Math.round(a.filter(s=>s.done).length/a.length*100):0;return `<button type="button" class="ref-goal-row" data-elara-tab="goals"><span>${icon('goals')}</span><span><strong>${esc(g.title||g.name||'هدف')}</strong><i class="elara-track"><i style="width:${p}%"></i></i></span><b>${fa(p)}٪</b></button>`}).join(''):'<p class="ref-empty">هنوز هدفی ثبت نشده است.</p>'}
function renderMissions(){const host=$('elara-home-missions');if(!host)return;const compact=isMobile(),ms=arr(window.ElaraMissions?.snapshot?.()).slice(0,compact?2:3);host.innerHTML=ms.length?ms.map(m=>`<div class="ref-mission-row"><span class="ref-mission-check" aria-hidden="true">${m.completed?'✓':'○'}</span><div><strong>${esc(m.name||'مأموریت')}</strong><small>${compact?'':esc(m.detail||'')+(m.detail?' · ':'')}${fa(m.amount)} / ${fa(m.target)}</small></div><b>+${fa(m.rewardXp)} XP</b></div>`).join(''):'<p class="ref-empty">مأموریتی برای نمایش نیست.</p>'}
...
function streakView(){const root=$('ref-streak-card');if(!root)return;const n=streak(),dates=activityDates(),d=new Date();d.setHours(12,0,0,0);const days=Array.from({length:7},(_,i)=>{const v=new Date(d);v.setDate(v.getDate()-6+i);const key=iso(v),doneToday=dates.has(key);return `<span class="ref-streak-day ${doneToday?'done':''} ${i===6?'today':''}"><b>${v.toLocaleDateString('fa-IR',{weekday:'narrow'})}</b><strong>${v.toLocaleDateString('fa-IR',{day:'numeric'})}</strong></span>`}).join(''),summary=`<div class="ref-streak-summary">${art(UI_ASSETS.flame,'ref-streak-flame','')}<strong>${fa(n)}</strong><small>روز تداوم</small></div>`;root.innerHTML=summary+`<div class="ref-streak-days">${days}</div>`;const desktop=$('ref-desktop-streak');if(desktop)desktop.innerHTML=`${art(UI_ASSETS.flame,'ref-streak-flame ref-streak-flame-desktop','')}<span><strong>${fa(n)}</strong><small> روز تداوم</small></span>`}
function accountHeader(){const bar=document.querySelector('.topbar'),actions=bar?.querySelector('.topbar-actions'),leading=bar?.querySelector('.topbar-leading');if(!bar||!actions)return;bar.classList.add('ref-topbar');document.querySelectorAll('.elara-toolbar').forEach(x=>x.remove());if(leading&&!leading.querySelector('.ref-mobile-brand')){const brand=element('a','ref-mobile-brand','ref-mobile-brand');brand.href='#home';brand.setAttribute('aria-label','Elara Space');brand.innerHTML='<img src="assets/logo.svg" alt=""><span><b>Elara</b><small>زندگی بهتر، هر روز</small></span>';leading.append(brand)}
 ...
 account.classList.toggle('ref-account-brand-duplicate',name.trim().toLocaleLowerCase()==='elara');account.innerHTML=`<span class="ref-account-avatar">${avatar?`<img src="${esc(avatar)}" alt="">`:esc(name.trim()[0]||'E')}</span><span class="ref-account-copy"><strong>${esc(name)}</strong><small>سطح ${fa(level)} · ${fa(xp)} XP</small><span class="ref-xp-track"><i style="width:${Math.min(100,(xp%1000)/10)}%"></i></span></span>`;
...
function render(){homeStructure();libraryFocus();renderTasks();renderHabits();renderGoals();renderMissions();wellness();streakView();accountHeader();const p=$('panel-home');if(p){const h=p.querySelector('.hero-copy h1');if(h)h.textContent='قدم‌های کوچک، آینده‌های بزرگ می‌سازند.'}}
let scheduled=false;function schedule(){if(scheduled)return;scheduled=true;setTimeout(()=>{scheduled=false;render()},90)}
function actions(e){
 const task=e.target.closest('[data-ref-task]');if(task){const target=document.querySelector(`#task-list [data-action="toggle-task"][data-id="${CSS.escape(task.dataset.refTask)}"]`);if(target)target.click();else window.ElaraOpen?.('tasks');schedule();return}
 const habit=e.target.closest('[data-ref-habit]');if(habit){const target=document.querySelector(`#habit-list [data-action="toggle-habit"][data-id="${CSS.escape(habit.dataset.refHabit)}"]`);if(target)target.click();else window.ElaraOpen?.('habits');schedule();return}
