...
## افزودهٔ Social — Friends Presence / Avatar Strip / Stories

- ⬜ بالای Friends یک **Avatar Strip افقی** از دوستان پذیرفته‌شده با عکس پروفایل، نام کوتاه/tooltip و بازشدن Profile.
- ⬜ **Online Presence** و **Recent Activity** دو وضعیت جدا باشند؛ Presence با timestamp/TTL معتبر و Activity فقط از eventهای opt-in.
- ⬜ Privacy برای Online/Last Active: everyone/friends/nobody یا مدل معادل نهایی؛ Block/Mute باید نمایش Presence/Activity را محدود کند.
- ⬜ Mobile: اسکرول افقی روان، اندازهٔ Avatar مناسب لمس، snap/overflow بدون شکستن Layout؛ Desktop: hover/focus/keyboard.
- ⬜ زیرساخت Avatar Strip برای **Stories** آماده باشد: seen/unseen ring، expiry، viewer privacy، mute/report، moderation و بعداً upload/media limits.
- ⬜ Story قابلیت آینده است؛ تا وقتی backend، storage rules، moderation و privacy کامل نشده، UI ساختگی یا Story نمونه به حساب واقعی نمایش داده نشود.


## P0 فوری — قبل از هر توسعهٔ جدید

- 🔴 **Focus Restart Dialog:** تمام Restart/Reset/Cancel confirmationها فقط با Elara Dialog centered و Theme-aware؛ هیچ متن confirm داخل document flow یا پایین viewport رندر نشود.
- 🔴 **Task Completion Event History:** Completion از lifecycle خود Task جدا شود؛ Delete کردن Task تکمیل‌شده نباید completed count / mission / dashboard history را کم کند. Undo completion تنها مسیر کم‌کردن همان event باشد.
- 🔴 **Topbar Three-dot Trigger:** تنها Trigger منوی خصوصی `⋯` داخل topbar و دقیقاً کنار Dark/Light باشد؛ Mobile و Desktop. Trigger شناور پایین/میانه ممنوع.
- 🔴 این سه مورد باید قبل از Social/Club/Theme/Message Center جدید تست و بسته شوند.
